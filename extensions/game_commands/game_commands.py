...
class GameCommands(commands.Cog):
    ...
    async def process_command(self, name, message, *, officer: bool = False):
        ...
        try:
            ...
            if cmd:
                await cmd(name, args, officer=officer)
            else:
                await self.unknown_command(name, args, officer=officer)
        except Exception as e:
            await self.send_chat_message(name, f"Error processing command '{command}': {str(e)}", officer=officer)
    ...
    async def on_command_error(self, ctx, error):
        ...
        if isinstance(error, commands.CommandNotFound):
            if ctx.channel.id == DiscordConfig.channel:
                ...
                await self.on_hypixel_guild_message("@" + ctx.author.name, ctx.message.content)
            elif ctx.channel.id == DiscordConfig.officerChannel:
                print(f"{Color.MAGENTA}Game Commands{Color.RESET} > Invoking from Discord")
                await self.on_hypixel_guild_officer_message("@" + ctx.author.name, ctx.message.content)
    ...
    async def help(self, name, args, *, officer: bool = False):
        if len(args) == 0:
            return await self.send_chat_message(name, "Commands: " + ", ".join([PREFIX + k for k in self.commands.keys()]), officer=officer)
        command = args[0]
        if command not in self.commands:
            return await self.send_chat_message(name, f"Unknown command: {command}.", officer=officer)
        if command == "help":
            return await self.send_chat_message(name, f"Usage: {PREFIX}help [command]", officer=officer)
        if command == "level":
            return await self.send_chat_message(name, f"Usage: {PREFIX}level [player] [profile]", officer=officer)
        if command == "slayers":
            return await self.send_chat_message(name, f"Usage: {PREFIX}slayers [player] [profile] [boss]", officer=officer)
        return await self.send_chat_message(name, f"Command '{command}' has no help text.", officer=officer)

    async def level(self, name, args, *, officer: bool = False):
        if len(args) == 0 and name.startswith("@"):
            return await self.send_chat_message(name, f"Must provide player name for Discord commands.", officer=officer)
    ...
    async def slayers(self, name, args, *, officer: bool = False):
        if len(args) == 0 and name.startswith("@"):
            return await self.send_chat_message(name, f"Must provide player name for Discord commands.", officer=officer)
        player = args.pop(0) if args else name
        profile = args.pop(0) if args else None
        ...
        if slayers is None:
            return await self.send_chat_message(name, f"{player} has no slayer data", officer=officer)
        if selected_slayer is None:
            ...
            return await self.send_chat_message(name, message, officer=officer)
        else:
            if selected_slayer not in bosses:
                return await self.send_chat_message(name, f"Invalid slayer: {', '.join(bosses)}.", officer=officer)
            boss = selected_slayer
            boss_data = slayers.get(selected_slayer)
            if boss_data is None:
                return await self.send_chat_message(name, f"{player} has no data for {selected_slayer.capitalize()}.", officer=officer)
            message = f"{player} ({profile['cute_name']}): "
